...
public class GameSave {
    ...
    public static void guardar(GamePanel gp) {
        ...
        for (Tower t : gp.towers) {
            ...
            if (t instanceof BasicTower)
                tipo = 1;
            else if (t instanceof SlowDownTower)
                tipo = 2;
            else if (t instanceof MagicTower)
                tipo = 3;
    ...
    private static Tower crearTorre(int tipo, int gridX, int gridY, GamePanel gp) {
        switch (tipo) {
            case 1:
                return new BasicTower(gridX, gridY, gp.tower1Img);
            case 2:
                return new SlowDownTower(gridX, gridY, gp.tower4Img);
            case 3:
                return new MagicTower(gridX, gridY, gp.tower3Img);
            case 4:
                return new SniperTower(gridX, gridY, gp.tower2Img);
            default:
                return null;
        }
    }
    ...
    private static Enemy crearEnemigo(int tipo, GamePanel gp) {
        switch (tipo) {
            case 1:
                return new NormalEnemy(0, 4 * gp.TILE_SIZE, gp.normalenemyImg);
            case 2:
                return new FastEnemy(0, 4 * gp.TILE_SIZE, gp.fastEnemyImg);
            case 3:
                return new TankEnemy(0, 4 * gp.TILE_SIZE, gp.tankEnemyImg);
            case 4:
                return new Witch(0, 4 * gp.TILE_SIZE, gp.witchImg);
            case 5:
                return new NIGROMANTE(0, 4 * gp.TILE_SIZE, gp.NIGROMANTEImg);
            default:
                return null;
        }
    }
